...
public class PageDto {
	...
	public PageDto(Criteria criteria, int total) {
		...
		// 실제 끝 페이지 계싼(총 글의 수 / 한페이지당 출력될 글의 수 --> 올림한 정수)
		// 글이 53개면 53/5=10.6 -> 올림 11로 변환한 값이 실제 끝 페이지수가 됨
		
		if(realEndPage < this.endPage) {
			this.endPage = realEndPage;
		} // 처음 계산한 마지막 페이지의 값인 endPage 값보다 realEndpage값이 작게 나오면 그 값으로 대신해야
		  // 실제 마지막 페이지값이 마지막 페이지 번호로 출력됨
		
		this.prev = this.startPage > 1; // startPage는 1 6 11 16 21...순서로 나가므로 1보다 크면 prev가 존재해야함
